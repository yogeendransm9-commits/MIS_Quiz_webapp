...
dashboard is password-protected (password stored as env QUIZ_ADMIN_PASSWORD).
All tables are shared/public for this one event, so RLS allows anon+authenticated
CRUD. Server-side answer validation + single-answer enforcement via SECURITY
DEFINER RPC (submit_answer).
